"""
Run 2 sequential federated PatchCore clients (no Flower) on two datasets:
- data/Pompe
- data/Radiateur

Flow:
1) start server (expects clients 1 and 2)
2) client1 upload (Pompe)
3) client2 upload (Radiateur)
4) client1 eval (Pompe)    # eval uses global bank for its categories
5) client2 eval (Radiateur)
6) shutdown server

Notes:
- Categories are inferred as folder names (Pompe, Radiateur).
- Uses k-center for BOTH client coreset + server aggregation.
- Validation split is ignored (dataset loader uses training/ + test/).
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run 2-client sequential federated PatchCore on Pompe/Radiateur")
    ...
    p.add_argument("--dataset-root", type=Path, default=Path("data"))
    p.add_argument("--artifacts-dir", type=Path, default=Path("federated_sequentially/artifacts/federated_sequential/pompe_radiateur_2clients"))
    p.add_argument("--logs-dir", type=Path, default=Path("federated_sequentially/logs/federated_sequential/pompe_radiateur_2clients"))
...
def main() -> None:
    ...
    clients = [
        {"client_id": 1, "categories": ["Pompe"]},
        {"client_id": 2, "categories": ["Radiateur"]},
    ]
