"""Run the sequential federated PatchCore pipeline (2 clients).

This script:
1) starts server_sequential.py
2) runs 2 clients sequentially in upload mode (client1 -> client2)
3) runs 2 clients sequentially in eval mode (client1 -> client2)
4) shuts down the server

Config:
- dataset root: data/xfk3/
- split train data deterministically across the 2 clients via --train-partition-id/--train-num-partitions
- coreset method: kcenter (clients) + server k-center aggregation
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run 2-client sequential federated PatchCore pipeline")
    ...
    p.add_argument("--dataset-root", type=Path, default=Path("data/xfk3/"))
    p.add_argument("--artifacts-dir", type=Path, default=Path("federated_sequentially/artifacts/federated_sequential/xfk3_2clients"))
    p.add_argument("--logs-dir", type=Path, default=Path("federated_sequentially/logs/federated_sequential/xfk3_2clients"))
...
def main() -> None:
    ...
    clients = [
        {"client_id": 1, "partition_id": 0, "num_partitions": 2},
        {"client_id": 2, "partition_id": 1, "num_partitions": 2},
    ]
    ...
    categories = sorted([p.name for p in args.dataset_root.iterdir() if p.is_dir()])
