...
\begin{itemize}
    ...
    \item Communication protocol: customr framework based on pickle RPC
\end{itemize}
...
The tightest $(\varepsilon, \delta)$-DP bound is obtained by choosing the order that minimizes the grid of Renyi orders $\alpha \in -1.25,\ 1.5,\ 2,\ 3,\ 4,\ 5,\ 8,\ 10,\ 16,\ 32,\ 64,\ 128\}$, where $T$ is the number of composition steps (equivalent to the number of categories released per client). The accountant chooses $\alpha = 16$ as the best order under the experimental configuration ($\sigma \approx 4.84$, $T = 2$, $\delta = 10^{-5}$), resulting in a total $\varepsilon \approx 1.45$. The grid ensures that the minimum is not overlooked at either extreme by covering both low orders, where the divergence term predominates, and high orders, where the composition term predominates.
...
We track detailed ablation results in Tables \ref{tab:full_ablation_auroc} and \ref{tab:full_ablation_pixel_auroc}, more specifically image level and pixel level AUROC scores for each label and average across all labels. From these detailed results we distill the following key findings, first at image leve:
\begin{itemize}
