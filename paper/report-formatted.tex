...
The 10% coreset sampling ratio is in line with the original PatchCore recommendation~\citep{Roth2022TotalRecall}, which showed that keeping a tenth of the full patch feature set gives enough coverage of the nominal distribution while keeping memory and nearest-neighbor search manageable during inference time. At this ratio, the biggest category (underbody_screw, 373 training images) gives about $3 \times 10^4$ patches before subsampling. This is a bank size where k-center selection is still possible in the chunked implementation (coreset_chunk_size = 8192).
Greedy k-center selection needs to calculate the distance between the candidate features and the currently chosen set over and over again. To avoid making the full $N \times N$ distance matrix, the implementation works on features in groups of 8192 rows at a time, updating a running minimum-distance vector. This number strikes a balance between GPU memory pressure and iteration overhead, and it is in line with the typical $L_2$-distance batch sizes used in PatchCore-style retrieval pipelines.
...
The tightest $(\varepsilon, \delta)$-DP bound is obtained by choosing the order that minimizes the grid of Rényi orders $\alpha \in −1.25,\ 1.5,\ 2,\ 3,\ 4,\ 5,\ 8,\ 10,\ 16,\ 32,\ 64,\ 128\}$, where $T$ is the number of composition steps (equivalent to the number of categories released per client). The accountant chooses $\alpha = 16$ as the best order under the experimental configuration ($\sigma \approx 4.84$, $T = 2$, $\delta = 10^{-5}$), resulting in a total $\varepsilon \approx 1.45$. The grid ensures that the minimum is not overlooked at either extreme by covering both low orders, where the divergence term predominates, and high orders, where the composition term predominates.
...
Standard DP and RDP yield comparable utility outcomes, with RDP exhibiting a marginally inferior image-level AUC on average (0.57 versus 0.58), yet a slightly superior pixel-level ROC AUC in specific categories (e.g., \texttt{engine\_wiring}: 0.67 versus 0.65, \texttt{pipe\_staple}: 0.54 versus 0.52). The practical distinction between the two mechanisms is minimal given the selected parameterization. The primary theoretical advantage of RDP—stricter composition bounds—does not yield a quantifiable utility increase in this context, as the principal expense is the noise multiplier $\sigma \approx 4.84$, rather than composition across rounds. Both versions fail on \texttt{tank\_screw}, where the AUC for RDP image A drops to 0.42, which is lower than random chance. This means that the discriminative features in this category are mostly found in a low-amplitude subspace that is more affected by high-variance Gaussian perturbations. Pixel AP drops to almost zero in all categories when either DP variant is used. This means that spatial localization doesn't really work at this privacy budget.
...
Because it maintains the natural data distribution across subgroups while maintaining per-group k-center diversity, the proportional allocation mode is employed in all reported experiments. Each subgroup $k$ receives $M_k = \lfloor M \cdot N_k / N \rfloor$ slots since the target coreset size $M$ is inherited from the global \texttt{coreset\_sampling\_ratio} $= 0.1$. To keep the fairness pass computationally lightweight, random sampling within each subgroup is used at the intra-group level instead of k-center; the per-group k-center step, which comes before server aggregation, satisfies the global diversity objective.
...
With low-noise corruption, the image-level ROC AUC only goes down a little across all categories ($\text{avg.}\ 0.74 \to 0.72$). This shows that the k-center memory bank has enough coverage to handle a small shift in distribution at test time. With medium noise, the performance drops by about $0.01$–$0.02$ per category, but it stays in the same range. High noise is the first level where you can see real degradation: at the image level, \texttt{tank\_screw} drops from $0.62$ to $0.50$ and \texttt{pipe\_clip} drops from $0.59$ to $0.56$. The pixel ROC AUC also drops more broadly ($\text{avg.}\ 0.87 \to 0.81$). 
The underbody categories stay strong even when there is a lot of noise (image AUC $\geq 0.93$ for \texttt{underbody\_pipes}), which makes sense because their larger training sets cover more memory banks. Pixel AP is more sensitive than ROC AUC at all noise levels. The only exception is \texttt{underbody\_screw}, which keeps a strong pixel AP ($0.38$ at high noise vs. $0.29$ at baseline). Overall, the federated pipeline is fairly strong against moderate input corruption. As noise increases, localization metrics get worse faster than image-level discrimination.
...
\\
The dataset is described in a croissant file provided by Renault. For each of the six categories the croissant metadata points to a zip file hosted on Zenodo. We have used the exact structure provided by Renault and the only preprocessing that we did was to unzip the archives.
\\
